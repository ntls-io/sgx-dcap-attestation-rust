...
impl QuotingEnclave {
    pub unsafe fn new<P: AsRef<OsStr>>(filename: P) -> Result<QuotingEnclave, libloading::Error> {
        Ok(QuotingEnclave {
            qe_lib: unsafe { Library::new(filename) }?,
        })
    }

    pub unsafe fn get_target_info(&self) -> Result<sgx_target_info_t, libloading::Error> {
    ...
    pub unsafe fn get_quote_size(&self) -> Result<uint32_t, libloading::Error> {
...
mod tests {
    ...
    fn get_quoting_enclave() -> QuotingEnclave {
        // TODO: Use once cell to ensure only one quoting enclave can be loaded for the tests.
        let result = unsafe { QuotingEnclave::new(get_mock_lib_path()) };
